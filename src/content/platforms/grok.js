// Fast GPTZero - Grok Content Script

console.log('Fast GPTZero: Grok content script loaded');
...
function init() {
  observeMessages();
  console.log('Fast GPTZero: Initialized for Grok');
}
...
function addButtonsToMessages() {
  // Grok uses specific selectors - it's part of X/Twitter interface
  const messageSelectors = [
    '[data-testid="grok-message"]',
    '.grok-response',
    '[class*="GrokMessage"]',
    '[data-grok-message]',
    'article[role="article"]' // X/Twitter article structure
  ];
  ...
  messageSelectors.forEach(selector => {
    ...
    messages.forEach(message => {
      // Check if this is a Grok response (not user message)
      const isUserMessage = message.querySelector('[data-testid="user-message"]') ||
                           message.closest('[data-user-message]') ||
      ...
      const contentArea = message.querySelector('[data-testid="tweetText"], .message-content, [class*="content"]') || message;
...
function handleRevisionRequest() {
  ...
  // Find Grok's input area
  const inputArea = document.querySelector('[contenteditable="true"], textarea, input[type="text"]');
...
function handleSendSelected() {
  ...
  const inputArea = document.querySelector('textarea, [contenteditable="true"], input[type="text"]');
  if (!inputArea) {
    showToast('Could not find input area', 'error');
    return;
  }
